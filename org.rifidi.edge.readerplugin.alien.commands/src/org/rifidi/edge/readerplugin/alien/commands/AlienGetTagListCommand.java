...
public class AlienGetTagListCommand extends AbstractAlien9800Command {
	...
	public void execute() throws TimeoutException {
		try {
			...
			TagReadEvent tagInfo = null;
			for (AlienTag tag : tagList) {
				tagInfo = factory.getTagReadEvent(tag);			
			}
			IoTSensedEvent rawSensedEvent = new IoTSensedEvent(tagInfo.getTag(), reader.get(), 20.0, System.currentTimeMillis());
			rawSensedEvent.addExtraInformation("temperature", 20.0);
			rawSensedEvent.addExtraInformation("humidity", 30.0);
			rawSensedEvent.addExtraInformation("light", 40.0);
			sensorSession.getSensor().sendEvent(rawSensedEvent);
