...
public class ObixEventListener implements StatementAwareUpdateListener {
	...
	private static final Log logger =
			LogFactory.getLog(ObixEventListener.class);
	...
	public void update(EventBean[] arg0, EventBean[] arg1, EPStatement arg2,
		...
		if ((arg0 != null)
				&& (ecspecMgmt.getSubscriptionsByLRName("Obix_1") != null)) {
			IoTSensedEvent rawIoTEvent =
					(IoTSensedEvent) arg0[0].getUnderlying();
			...
			ECReportGroupListMemberExtension ext =
					new ECReportGroupListMemberExtension();
			ext.setFieldList(new ECReportGroupListMemberExtension.FieldList());

			Iterator iterator =
					rawIoTEvent.getExtraInformation().keySet().iterator();
			while (iterator.hasNext()) {
				String eventName = iterator.next().toString();
				System.out.println("[IoT Event] " + eventName + " = "
						+ rawIoTEvent.getExtraInformation().get(eventName));
			...
			for (String aSub : ecspecMgmt.getSubscriptionsByLRName("Obix_1")) {
				...
				System.out.println("Sending obix report to Cap App: " + host
						+ ":" + port);
				try {
